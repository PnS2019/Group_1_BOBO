...
for x in range(0, num_epochs):
    ...
    #Shuffle lists to differ each epoch
    skshuffle(train_X, train_y)

    #Iterate over batches
    for y in range(0, int(num_train_samples/batch_size)):

        samples = train_X[batch_size * y :batch_size * (y + 1)]
        lables = np.expand_dims(train_y[batch_size * y :batch_size * (y + 1)], 1)

        #print(np.shape(samples))
        #print(np.shape(lables))

        #Train
        train_function((samples, lables))




#TEST
#No need for epochs since network is already trained^^
for y in range(0, int(num_test_samples/batch_size)):
    samples = test_X[batch_size * y :batch_size * (y + 1)]
    lables = np.expand_dims(test_y[batch_size * y :batch_size * (y + 1)], 1)

    #Test
    test_function((samples, lables))
