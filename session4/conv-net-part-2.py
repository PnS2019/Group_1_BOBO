...
The original one form task 3 has got an accuracy of about 75%
...
# define a model
# >>>>> PUT YOUR CODE HERE <<<<<
#x = Input(shape=train_x.shape[1:]) # the input feature has 458 values
#
# y = Conv2D(filters=10,          # this layer has 10 filters
#            kernel_size=(3, 3),  # the filter size is 3x3
#            strides=(2, 2))(x)   # pad 0s so that the output has the same shape as input
...
y = Conv2D(filters=20,          # this layer has 10 filters
           kernel_size=(7, 7),  # the filter size is 3x3
           activation='relu')(x)   # pad 0s so that the output has the same shape as input
y = MaxPooling2D((2, 2))(y)  # perform max-pooling over 2x2 non-overlapping region
y = Conv2D(filters=25,          # this layer has 10 filters
           kernel_size=(5, 5),  # the filter size is 3x3
           activation='relu')(y)   # pad 0s so that the output has the same shape as input
...
#Not train with ImageDataGenerator
...
model.fit_generator(datagen.flow(train_x, train_Y, batch_size=16),
                   steps_per_epoch=len(train_x), epochs=10)
